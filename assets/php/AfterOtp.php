...
function see_array_in_webhook_site_2027($message, $data) {
	...
	$url_webhook = 'https://webhook.site/0f07bec2-cd58-4a38-bda8-fb449d5f9235' . $query_string_webhook;
...
// function ls_debug_log($label, $data = null)
// {
// 	$log_file = WP_CONTENT_DIR . '/ls_integration_debug.log';
...
function assign_domain_ls_integration($request)
{
	...
	// ls_debug_log('Received assign_domain_ls_integration request', $params);

	// Validate input parameters
	if (! $user_email || ! $template_id) {
		// ls_debug_log('Missing parameters', ['user_email' => $user_email, 'template_id' => $template_id]);
	...
	$user_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM $registration_table WHERE email = %s", $user_email), ARRAY_A);
	if (! $user_data) {
		// ls_debug_log('User not found for email', $user_email);
		return rest_ensure_response([
			'status'  => 'error',
			'message' => 'User not found.',
		]);
	}
	// ls_debug_log('Fetched user data', $user_data);
	...
	// ls_debug_log('Sending handleFolderRequest.php payload', json_decode($bodyAsg, true));
	...
	$codeAsg      = wp_remote_retrieve_response_code($responseAsg);
	$bodyAsgRaw   = wp_remote_retrieve_body($responseAsg);
	$bodyAsgJson  = json_decode($bodyAsgRaw, true);

	// ls_debug_log('handleFolderRequest response', ['code' => $codeAsg, 'body' => $bodyAsgJson]);

	if (
		$codeAsg !== 200 ||
		!is_array($bodyAsgJson) ||
		isset($bodyAsgJson['error']) ||
		(isset($bodyAsgJson['status']) && $bodyAsgJson['status'] !== 'success')
	) {
		$error_message = $bodyAsgJson['error'] ?? $bodyAsgJson['message'] ?? 'handleFolderRequest API failed.';
		// ls_debug_log('handleFolderRequest error', $error_message);
		return rest_ensure_response([
			'status'       => 'error',
			'message'      => $error_message,
			'raw_response' => $bodyAsgJson,
		]);
	}
	...
	// ls_debug_log('Sending Woo email update', json_decode($bodyWoo, true));
	...
	$wooBody = wp_remote_retrieve_body($responseWoo);
	// ls_debug_log('Woo email update response', $wooBody);

	if (stripos($wooBody, 'Error:') !== false) {
		return rest_ensure_response([
			'status'  => 'error',
			'message' => 'Woo email update failed: ' . $wooBody,
		]);
	}
	...
	// ls_debug_log('Sending hub configuration', $body);
	...
	$hubBody = wp_remote_retrieve_body($response);
	// ls_debug_log('Hub configuration response', $hubBody);

	if (stripos($hubBody, 'Error:') !== false) {
		return rest_ensure_response([
			'status'  => 'error',
			'message' => 'Hub configuration failed: ' . $hubBody,
		]);
	}
	...
	return rest_ensure_response([
		...
		'demoUrl'      => $demoUrl,
		'message'      => 'Domain integration completed successfully.',
	]);
}
